...
RDEPEND="
	...
	introspection? ( >=dev-libs/gobject-introspection-1.50.0 )
	mbim? ( >=net-libs/libmbim-1.16.0 )
	policykit? ( >=sys-auth/polkit-0.113[introspection] )
...
src_configure() {
	gnome2_src_configure \
		...
		$(usex systemd --with-systemd-suspend-resume --with-systemd-journal) \
		$(use_with qmi) \
		$(use_enable vala)
}
